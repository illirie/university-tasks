...
try:
    connection=cx_Oracle.connect('tkd_6309/241002')
...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.setGeometry(500, 250, 750, 500)  # left, top, width, height
    ...
    def show_modif_window(self, action: str) -> None:
        ...
        for condition_field in self.modif_window.qlineedit_conditions_part:
            condition_field.setEnabled(action == 'upd' or action == 'del')

        for new_values_field in self.modif_window.qlineedit_new_values_part:
            new_values_field.setEnabled(action == 'upd' or action == 'add')

        self.modif_window.qlineedit_new_values[0].setEnabled(action != 'add' and action != 'del')
        self.modif_window.qlineedit_new_values_part[0].setEnabled(action != 'add' and action != 'del')
    ...
    def create_layout(self) -> QGridLayout:
        ...
        for i in range(0, len(buttons)):
            ...
            buttons[i].setFixedHeight(150)
            layout.addWidget(buttons[i], (i+2) // 2, i % 2)
...
def main() -> None:    
    ...
    main_window.setWindowTitle('Database Options')
    main_window.show()
    sys.exit(app.exec_())
